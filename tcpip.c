...
#define P1_PORT 30000
#define P2_PORT 30001
#define P3_PORT 30002
...
void usage()
{
  fprintf(stderr, "Usage: tcpip p1_num p2_num p3_num\n");
  exit(1);
}
...
int open_socket()
{
  ...
  if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (char *)&reuse, sizeof(int)) == -1)
    error("Can't make socket reusable");
  return s;
}

/**
 * Setup a socket to wait for a connection
 * and return the file descriptor of the socket that was accepted.
 */
int wait_for_connection(int port)
{
  int listener_d = open_socket();
...
/**
 * Establish a connection to localhost at the specified port
 * and return the file descriptor of the socket
 */
int connect_to_port(int port)
{
  int sockout = open_socket();
  ...
  if (connect(sockout, (struct sockaddr *)&si, sizeof(si)) == -1)
...
int main(int argc, char *argv[])
{
  ...
  if (!pid1) {
    ...
    // accept connection from p3
    int p3_connection = wait_for_connection(P1_PORT);
    // connect to p2
    int p2_connection = connect_to_port(P2_PORT);

    int sum1 = p1_add(p1_num, p2_connection, p3_connection);
    printf("Sum calculated by party 1: %d\n", sum1);

    int prod1 = p1_mult(p1_num, p2_connection, p3_connection);
  ...
  if (!pid2) {
    ...
    int p1_connection = wait_for_connection(P2_PORT);
    // connect to p3
    int p3_connection = connect_to_port(P3_PORT);

    int sum2 = p2_add(p2_num, p1_connection, p3_connection);
    printf("Sum calculated by party 2: %d\n", sum2);

    int prod2 = p2_mult(p2_num, p1_connection, p3_connection);
  ...
  if (!pid3) {
    ...
    // connect to p1
    int p1_connection = connect_to_port(P1_PORT);
    // accept connection from p3
    int p2_connection = wait_for_connection(P3_PORT);

    int sum3 = p3_add(p3_num, p1_connection, p2_connection);
    printf("Sum calculated by party 3: %d\n", sum3);

    int prod3 = p3_mult(p1_connection, p2_connection);
